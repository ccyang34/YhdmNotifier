    ...
    for item in anime_items:
        ...
        if ((title in exact_titles) or any(keyword in title for keyword in keywords)) and update_date in valid_dates:
            ...
            if update_date == today:
                updates.append(
                    f"<font size=\"6\" color=\"red\"><a href=\"alook://{link}\" style=\"color: red; text-decoration-color: red;\"><font color=\"red\">{title}</font></a></font>\n {episode} 🔥 更新日期：{update_date} {weekday_zh}\n\n")
            else:
                updates.append(
                    f"<font size=\"6\" color=\"orange\"><a href=\"alook://{link}\" style=\"color: orange; text-decoration-color: orange;\"><font color=\"orange\">{title}</font></a></font>\n {episode} 🔥 更新日期：{update_date} {weekday_zh}\n\n")
    return updates
